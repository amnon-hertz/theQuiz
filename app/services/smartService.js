app.factory("smartService",function($http, $q){
    ...
    function prepareQuiz(quiz) {
       $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/question?quizId='+quiz.id).then(function(response) {
         for (i=0;i< response.data.length ; i++){ 
           var question = response.data[i];
           question.status = "INIT";
            $http.put('https://json-server-heroku-uoqavpbzgk.now.sh/question/'+question.id, question).then(function(resp2) {                           
                })           
           }
       });

       $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/member?quizId='+quiz.id).then(function(resp3) {
         for (i=0;i< resp3.data.length ; i++){ 
           var member = resp3.data[i];
            $http.delete('https://json-server-heroku-uoqavpbzgk.now.sh/member/'+member.id).then(function(resp4) {                           
                })           
           }
       });
    }
    ...
    function getNextQuestion(quiz) {
        async = $q.defer()
        $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/question?quizId=' + quiz.id + "&status=INIT").then(function(response) {
   ...
   function getActiveQuiz() {
         $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/quiz').then(function(response) {
    ...
    function calcSummeries(quiz) {
       ...
       $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/question?quizId='+quiz.id).then(function(response) {
         for (i=0;i< response.data.length ; i++){ 
          ...
          $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/member?quizId='+quiz.id).then(function(resp2) {
    ...
    function getCurrQuestion(quiz) {
     ...
     function updAnswerOfMember(member) {
        $http.put('https://json-server-heroku-uoqavpbzgk.now.sh/member/'+member.id, member).then(function(resp2) {                           
                })       
    }
    ...
    function calcPoints(quiz, question) {
       ...
       $http.get('https://json-server-heroku-uoqavpbzgk.now.sh/member?quizId='+quiz.id).then(function(resp2) {
                ...
                for (i=0; i< memberWide.length ; i++) {
                  ...
                  $http.put('https://json-server-heroku-uoqavpbzgk.now.sh/member/'+memberWide.id, memberWide).then(function(resp2) {                           
                  })                  
                }
